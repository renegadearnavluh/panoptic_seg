...
from my_ps.data_mini.cityscapes.augmentations import RandomCropWithInstance, AugInput, EvaluationCityscapesAugInput

__all__ = ["CityscapesPanopticDatasetMapper", "EvaluationCityscapesDatasetMapper"]

class EvaluationCityscapesDatasetMapper:
    """
    ...
    def __call__(self, dataset_dict):
        ...
        raw_img_extension = self.cfg.MODEL.DEPTH_MODALITY.RAW_IMG_EXTENSION
        if raw_img_extension=="png":
          depth = np.array(imageio.imread(dataset_dict["depth_file_name"]) , dtype=np.float32)
        elif raw_img_extension=="npy":
          depth = np.load(dataset_dict["depth_file_name"])
        else:
          print("{} not supported!".format(raw_img_extension))

        calibration_info = json.load(open(dataset_dict["calib_file"], "r"))
        dataset_dict["calib"] = calibration_info
        ...
        # aug_input = EvaluationCityscapesAugInput(image, depth=depth, sem_seg=sem_seg_gt)
...
class CityscapesPanopticDatasetMapper:
  ...
  def __call__(self, dataset_dict):
    ...
    aug_input = AugInput(image, depth=depth, sem_seg=sem_seg_gt, inst_map=inst_map)
    transforms = self.augmentations(aug_input)
    image, sem_seg_gt = aug_input.image, aug_input.sem_seg
    depth = aug_input.depth
