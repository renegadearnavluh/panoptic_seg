...
import random
random.seed(0)
from my_ps.utils.demo.colormap import random_color, _COLORS
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, alpha=0.8):
        ...
        stuff_and_thing_classes = self.metadata.get("thing_classes", None) + self.metadata.get("stuff_classes", None)
            
        labels = _create_text_labels(classes, scores, stuff_and_thing_classes)
        keypoints = predictions.pred_keypoints if predictions.has("pred_keypoints") else None

        if predictions.has("pred_masks"):
            masks = np.asarray(predictions.pred_masks)
            masks = [GenericMask(x.numpy(), self.output.height, self.output.width) for x in masks]
        else:
            masks = None
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            self.output.reset_image(
                self._create_grayscale_image(
                    (predictions.pred_masks.tensor.any(dim=0) > 0).numpy()
                    if predictions.has("pred_masks")
                    else None
                )
            )
    ...
    def draw_sem_seg(self, sem_seg, area_threshold=None, alpha=0.8):
        ...
        classes = self.metadata.stuff_classes + self.metadata.thing_classes
        colors = self.metadata.stuff_colors + self.metadata.thing_colors
        for label in filter(lambda l: l < len(classes), labels):
            try:
                mask_color = [x / 255 for x in colors[label]]
    ...
    def draw_panoptic_seg(self, panoptic_seg, segments_info, area_threshold=None, alpha=0.7):
        ...
        labels = _create_text_labels(
            category_ids, scores, classes, [x.get("iscrowd", 0) for x in sinfo]
        )

        colors = self.metadata.stuff_colors + self.metadata.thing_colors
        try:
            colors = [
                self._jitter([x / 255 for x in colors[c]]) for c in category_ids
            ]
        except AttributeError:
            colors = None
