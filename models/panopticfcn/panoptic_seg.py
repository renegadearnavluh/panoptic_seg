...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.FUSION_STAGE == "MID_FUSION":
            self.backbone = build_resnet_fusion_fpn_backbone(cfg, None, should_forward_fused_features=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT) if cfg.MODEL.FPN.TYPE == "FPN" else build_resnet_mid_fusion_fan_backbone(cfg, None, should_forward_fused_feat=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT)
        elif cfg.MODEL.FUSION_STAGE == "LATE_FUSION":
            self.backbone = build_backbone(cfg) if cfg.MODEL.FPN.TYPE == "FPN" else build_resnet_fan_backbone(cfg)
            if self.depth_enable and not self.depth_shared_weight:
                self.depth_backbone = build_backbone(cfg, modal="depth") 
                #if cfg.MODEL.FPN.TYPE == "FPN" else build_resnet_fan_backbone(cfg, modal="depth")
        elif cfg.MODEL.FUSION_STAGE == "MID_LATE_FUSION" and cfg.MODEL.FPN.TYPE == "FAN":
            ...
            raise ValueError(f"Unsupported fusion type: {cfg.MODEL.FUSION_STAGE} for {cfg.MODEL.FPN.TYPE}")

        # Initialize fusion module for Late Fusion
        if self.depth_enable and cfg.MODEL.FUSION_STAGE == "LATE_FUSION":
    ...
    def forward(self, batched_inputs):
        ...
        * Other information that's included in the original dicts, such as:
        ...
        if self.cfg.MODEL.DEPTH_MODALITY.ENABLED:
            ...
            if self.cfg.MODEL.FUSION_STAGE == "MID_FUSION" or self.cfg.MODEL.FUSION_STAGE == "MID_LATE_FUSION":
                features = self.backbone(images.tensor, depths.tensor)
                print("features after forwarding:")
                for level, feature_map in features.items():
                    print(f"Level: {level}")
                    print(f"  Shape: {feature_map.shape}")
                    print(f"  Mean: {feature_map.mean().item():.6f}")
                    print(f"  Max: {feature_map.max().item():.6f}")
                    print(f"  Min: {feature_map.min().item():.6f}")
                    print(f"  Number of NaNs: {torch.isnan(feature_map).sum().item()}")
                    print("-" * 40)
            else:
                ...
                print("features after forwarding late fusion:")
                for level, feature_map in features.items():
                    print(f"Level: {level}")
                    print(f"  Shape: {feature_map.shape}")
                    print(f"  Mean: {feature_map.mean().item():.6f}")
