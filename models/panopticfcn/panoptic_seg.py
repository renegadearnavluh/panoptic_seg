    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.FUSION_STAGE == "MID_FUSION":
            ...
            if self.depth_enable and not self.depth_shared_weight:
                self.depth_backbone = build_backbone(cfg, modal="depth") if cfg.MODEL.FPN.TYPE == "FPN" else build_resnet_fan_backbone(cfg, modal="depth")
        elif cfg.MODEL.FUSION_STAGE == "MID_LATE_FUSION" and cfg.MODEL.FPN.TYPE == "FAN":
            self.backbone = build_resnet_depth_enhanced_fan_backbone(cfg, None, should_forward_fused_feat=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT)
        else:
