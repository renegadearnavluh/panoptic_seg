...
from .fused_backbone_utils import build_resnet_with_fusion_backbone
from .backbone_utils import LastLevelP6P7fromP5
__all__ = ["build_resnet_fan_backbone", "build_retinanet_resnet_fan_backbone", "FAN"]

from .deform_conv_with_off import  ModulatedDeformConvWithOffAndMask
...
class FeatureAlign_V2(nn.Module):  # FaPN full version
    def __init__(self, in_nc=128, out_nc=128, norm=None):
        ...
        self.dcpack_L2 = ModulatedDeformConvWithOffAndMask(out_nc, out_nc, 3, stride=1, padding=1, dilation=1, deformable_groups=8,extra_offset_mask=True)
    ...
    def __init__(self, bottom_up, in_features, out_channels, norm="", top_block=None, fuse_type="sum"):
        ...
        Args:
            ...
            fuse_type (str): types for fusing the top down features and the lateral
                ...
                which takes the element-wise mean of the two.
        """
        super(FANWithFusion, self).__init__()
        assert isinstance(bottom_up, Backbone)
...
def build_resnet_mid_fusion_fan_backbone(cfg, input_shape=None, should_forward_fused_feat=False):

    if input_shape is None:
        input_shape = ShapeSpec(channels=len(cfg.MODEL.PIXEL_MEAN))

    bottom_up = build_resnet_with_fusion_backbone(cfg, input_shape, should_forward_fused_features=should_forward_fused_feat)
    
    in_features = cfg.MODEL.FPN.IN_FEATURES

    out_channels = cfg.MODEL.FPN.OUT_CHANNELS
    in_channels_p6p7 = out_channels
    backbone = FANWithFusion(bottom_up=bottom_up, in_features=in_features, out_channels=out_channels, norm=cfg.MODEL.FPN.NORM,
                     top_block=LastLevelP6P7fromP5(in_channels_p6p7, out_channels), fuse_type=cfg.MODEL.FPN.FUSE_TYPE, )
    return backbone



def build_resnet_fan_backbone(cfg, input_shape=None, modal='color'):
    """
    Args:
