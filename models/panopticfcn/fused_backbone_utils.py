...
class ResNetWithFusion(Backbone):
    ...
    def forward(self, rgb_input, depth_input):
        ...
        for i, (rgb_stage, depth_stage) in enumerate(zip(self.rgb_stages, self.depth_stages)):
            ...
            print(f"fused features shape, mean, std, max, min after stage {stage_name}: {fused_features.shape}, {fused_features.mean()}, {fused_features.std()}, {fused_features.max()}, {fused_features.min()}")
            print(f"rgb features shape, mean, std, max, min after stage {stage_name}: {rgb_features.shape}, {rgb_features.mean()}, {rgb_features.std()}, {rgb_features.max()}, {rgb_features.min()}")
            print(f"depth features shape, mean, std, max, min after stage {stage_name}: {depth_features.shape}, {depth_features.mean()}, {depth_features.std()}, {depth_features.max()}, {depth_features.min()}")
