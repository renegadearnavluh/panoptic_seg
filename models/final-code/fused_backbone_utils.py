...
class ResNetWithFusion(Backbone):
    def __init__(self, rgb_stem, depth_stem, rgb_stages, depth_stages, fusion_module_type, out_features, freeze_at=0, num_classes=None, should_forward_fused_features=False):
        ...
        self.bn_layers = nn.ModuleDict()  # Store BatchNorm layers
        ...
        for i, (rgb_blocks, depth_blocks) in enumerate(zip(rgb_stages, depth_stages)):
            ...
            self.add_module(f"{stage_name}depth", depth_stage)
            ...
            # **Add Layer Normalization after fusion**
            #self.fusion_modules[f"{stage_name}_ln"] = nn.LayerNorm([in_channels, 1, 1])  # Per-channel LN
            self.bn_layers[stage_name] = nn.BatchNorm2d(in_channels)
        ...
        print(f"stage names: {self.stage_names}, {len(rgb_stages)}, {len(depth_stages)}")
        print(f"Fused ResNet backbone created with out features {self._out_features} ")
    ...
    def forward(self, rgb_input, depth_input):
        ...
        for i, (rgb_stage, depth_stage) in enumerate(zip(self.rgb_stages, self.depth_stages)):
            ...
            #fused_features = self.fusion_modules[f"{stage_name}_ln"](fused_features)
            # Apply BatchNorm after fusion
            fused_features = self.bn_layers[stage_name](fused_features)
            if self.should_forward_fused_features:
                rgb_features = fused_features  # Feed fused features into the next stage
            print(f"fused features shape, mean, std, max, min after stage {stage_name}: {fused_features.mean()}, {fused_features.std()}, {fused_features.max()}, {fused_features.min()}")
            print(f"rgb features shape, mean, std, max, min after stage {stage_name}:  {rgb_features.mean()}, {rgb_features.std()}, {rgb_features.max()}, {rgb_features.min()}")
            print(f"depth features shape, mean, std, max, min after stage {stage_name}:{depth_features.mean()}, {depth_features.std()}, {depth_features.max()}, {depth_features.min()}")
