...
from .loss import sigmoid_focal_loss, weighted_dice_loss, weighted_dice_loss_depth_aware
...
from .depth_modal import depth_norm, average_depth_color_feature, ConcatModality, SAEFusion
from .fan_backbone_utils import build_resnet_mid_fusion_fan_backbone, build_resnet_fan_backbone
from .depth_enhanced_fan_backbone_utils import build_resnet_depth_enhanced_fan_backbone
from .depth_aware_heads import build_depth_aware_kernel_head
from .fused_backbone_utils import build_resnet_fusion_fpn_backbone
__all__ = ["PanopticFCN"]
...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.fusion_stage          = cfg.MODEL.FUSION_STAGE
        ...
        self.dcoord                = cfg.MODEL.KERNEL_HEAD.DCOORD
        ...
        self.depth_enable = cfg.MODEL.DEPTH_MODALITY.ENABLED

        self.depth_shared_weight = cfg.MODEL.DEPTH_MODALITY.SHARED_WEIGHT
        self.fpn_type = cfg.MODEL.FPN.TYPE
        # Select the appropriate backbone initialization
        if self.fusion_stage == "MID_FUSION":
            self.backbone = build_resnet_fusion_fpn_backbone(cfg, None, should_forward_fused_features=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT) if self.fpn_type == "FPN" else build_resnet_mid_fusion_fan_backbone(cfg, None, should_forward_fused_feat=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT)
        elif self.fusion_stage == "LATE_FUSION":
            self.backbone = build_backbone(cfg) if self.fpn_type == "FPN" else build_resnet_fan_backbone(cfg)
            if self.depth_enable and not self.depth_shared_weight:
                self.depth_backbone = build_backbone(cfg, modal="depth") if self.fpn_type == "FPN" else build_resnet_fan_backbone(cfg, modal="depth")
        elif self.fusion_stage == "MID_LATE_FUSION" and self.fpn_type == "FAN":
            self.backbone = build_resnet_depth_enhanced_fan_backbone(cfg, should_forward_fused_feat=cfg.MODEL.FPN.SHOULD_FORWARD_FUSED_FEAT)
        else:
            raise ValueError(f"Unsupported fusion type: {self.fusion_stage} for {self.fpn_type}")

        # Initialize fusion module for Late Fusion
        if self.depth_enable and self.fusion_stage == "LATE_FUSION":
            fusion_option = cfg.MODEL.DEPTH_MODALITY.FUSION_TYPE
            if fusion_option == "mean":
                self.fusion_modal = average_depth_color_feature
            elif fusion_option == "concat":
                self.fusion_modal = ConcatModality().to(self.device)
            elif fusion_option == "sae":
                self.fusion_modal = SAEFusion().to(self.device)
            else:
                raise ValueError(f"Unsupported fusion option: {fusion_option}")
        

        if cfg.MODEL.FREEZE_COLOR_BACKBONE:
            self.backbone.requires_grad_(False)

        self.semantic_fpn          = build_semanticfpn(cfg, self.backbone.output_shape())
        self.position_head         = build_position_head(cfg)
        self.kernel_head           = build_kernel_head(cfg) if not self.dcoord else build_depth_aware_kernel_head(cfg)
        ...
        depth_mean = torch.Tensor([cfg.MODEL.DEPTH_MODALITY.DEPTH_MEAN]).to(self.device)
        depth_std = torch.Tensor([cfg.MODEL.DEPTH_MODALITY.DEPTH_STD]).to(self.device)
        self.normalizer_depth = lambda x, baseline, fx: depth_norm(x, baseline, fx, self.cfg.MODEL.DEPTH_MODALITY.NORMALIZATION, \
                                        depth_mean, depth_std, self.cfg.MODEL.DEPTH_MODALITY.MIN_DEPTH_NOISE_THRESHOLD, self.cfg.MODEL.DEPTH_MODALITY.MAX_DEPTH_NOISE_THRESHOLD)
    ...
    def forward(self, batched_inputs):
        ...
        * Other information that's included in the original dicts, such as:
        ...
        if self.depth_enable:
            ...
            raw_depths = [x["depth"].to(self.device) for x in batched_inputs]
            calib_params = [(torch.tensor(x["calib"]["extrinsic"]["baseline"]).to(self.device), torch.tensor(x["calib"]["intrinsic"]["fx"]).to(self.device)) for x in batched_inputs]
            depths = [self.normalizer_depth(x, calib[0], calib[1]) for x, calib in zip(depths, calib_params)]
            depths = [depth.repeat(3, 1, 1) for depth in depths]
            depths = ImageList.from_tensors(depths, self.backbone.size_divisibility)
            raw_depths = ImageList.from_tensors(raw_depths, self.backbone.size_divisibility)
            if self.fusion_stage == "LATE_FUSION":
                features = self.backbone(images.tensor)  # Late Fusion logic retained
                if not self.depth_shared_weight:
                    features_depth = self.depth_backbone(depths.tensor)
                else:
                    features_depth = self.backbone(depths.tensor)

                features = self.fusion_modal(features, features_depth)

            else:
                features = self.backbone(images.tensor, depths.tensor)
        else:
            features = self.backbone(images.tensor)
        ...
        assert raw_depths is not None, "To use this code, must provide depth information."
        raw_depths = raw_depths.tensor
        ...
        if self.dcoord:
            depths_in = [raw_depths for _ in self.in_feature]
            pred_centers, pred_regions, pred_weights = multi_apply(self.forward_single_level_depth_aware, features_in, depths_in)
        ...
        if self.training:
            ...
            return self.losses(pred_centers, pred_regions, pred_weights, encode_feat, gt_dict, raw_depths)
        else:
            return self.inference(batched_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat)
        
    def forward_single_level_depth_aware(self, feature, depth):
        pred_center, pred_region = self.position_head(feature)
        pred_weight = self.kernel_head(feature, depth, center_map=pred_center)
    ...
    def losses(self, pred_centers, pred_regions, pred_weights, encode_feat, gt_dict, raw_depths):
        """
        ...
        Args:
            ...
            raw_depths: raw depth information
        ...
        loss_thing = weighted_dice_loss_depth_aware(thing_pred, thing_gt, 
                                        gt_num=thing_gt_num,
                                        index_mask=thing_gt_idx,
                                        raw_depths=raw_depths,
                                        depth_aware_weight=self.cfg.MODEL.DEPTH_AWARE_WEIGHT,
