...
def load_cityscapes_panoptic(cfg, image_dir, gt_dir, gt_json, meta):
    """
    Args:
    ...
    for image_file, label_file, segments_info in files:
        ...
        # Depth information process
        depth_file = (
            image_file.replace("leftImg8bit", "disparity").split(".")[0] + ".{}".format(cfg.MODEL.DEPTH_MODALITY.RAW_IMG_EXTENSION)
        )
        # Camera calibration
        calib_file = (
            image_file.replace("leftImg8bit", "camera").split(".")[0] + ".json"
        )
        ret.append(
            {
...
_RAW_CITYSCAPES_PANOPTIC_SPLITS = {
    ...
    "my_cityscapes_fine_panoptic_val_separated": (
        "cityscapes/leftImg8bit/val",
        "cityscapes/gtFine/cityscapes_panoptic_val",
        "cityscapes/gtFine/cityscapes_panoptic_val.json",
    ),
    "my_cityscapes_fine_panoptic_val_split": (
        "cityscapes/leftImg8bit/val_split",
        "cityscapes/gtFine/cityscapes_panoptic_val_split",
        "cityscapes/gtFine/cityscapes_panoptic_val_split.json",
    ),
    "my_cityscapes_fine_panoptic_train_split": (
        "cityscapes/leftImg8bit/train_split",
        "cityscapes/gtFine/cityscapes_panoptic_train_split",
        "cityscapes/gtFine/cityscapes_panoptic_split.json",
    ),
    "my_cityscapes_fine_panoptic_val16": (
        "cityscapes/leftImg8bit/val16",
        "cityscapes/gtFine/cityscapes_panoptic_val16",
        "cityscapes/gtFine/cityscapes_panoptic_val16.json",
    ),
    "my_cityscapes_fine_panoptic_val2": (
        "cityscapes/leftImg8bit/val2",
        "cityscapes/gtFine/cityscapes_panoptic_val2",
        "cityscapes/gtFine/cityscapes_panoptic_val2.json",
    ),
    # "cityscapes_fine_panoptic_test": not supported yet
}


def register_all_cityscapes_panoptic(cfg, root):
    ...
    for key, (image_dir, gt_dir, gt_json) in _RAW_CITYSCAPES_PANOPTIC_SPLITS.items():
        ...
        DatasetCatalog.register(
            key, lambda x=image_dir, y=gt_dir, z=gt_json: load_cityscapes_panoptic(cfg, x, y, z, meta)
        )
