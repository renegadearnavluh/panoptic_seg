...
__all__ = ['sigmoid_focal_loss', 'weighted_dice_loss', 'weighted_dice_loss_depth_aware']

def weighted_dice_loss_depth_aware(
    prediction,
    target_seg,
    gt_num,
    index_mask,
    ...
    if mode == "thing":
        ...
        common_stride = 4  # Set hard number first, should get this number from config file
        AUXILIARY_TYPE = 1 # Set hard type first, should make it in config file if it helps
        WEIGHTED=depth_aware_weight

        scale = 1. / common_stride
        h_depth, w_depth = raw_depths.shape[-2:]
        new_h, new_w = int(h_depth * scale + 0.5), int(w_depth * scale + 0.5)
        raw_depths = F.interpolate(raw_depths.float().unsqueeze(0), size=(new_h, new_w),
                                  mode="bilinear", align_corners=False)[0]
        raw_depths = raw_depths.unsqueeze(1).unsqueeze(2).expand(n, instance_num, weighted_num, h, w)
        ...
        eps_ = 0.0001
        mask_ = target_seg <= eps_

        depth_to_obj = torch.zeros_like(raw_depths)
        depth_to_obj[torch.logical_not(mask_)] = raw_depths[torch.logical_not(mask_)]
        depth_to_obj = depth_to_obj.reshape(n, instance_num, weighted_num, h*w)
        depth_to_obj_mean = torch.mean(depth_to_obj, dim=-1)

        depth_weight_norm = torch.zeros_like(raw_depths)
        # depth_weight_norm[target_seg <= eps_] = torch.abs(depth_weight[target_seg <= eps_] - depth_to_obj_mean)
        depth_weight_norm[mask_] = raw_depths[mask_]
        depth_to_obj_mean = torch.unsqueeze(depth_to_obj_mean, -1)
        depth_to_obj_mean = torch.unsqueeze(depth_to_obj_mean, -1)
        # print(depth_weight_norm.shape, depth_to_obj_mean.shape); exit()
        depth_weight_norm -= depth_to_obj_mean
        tmp = torch.max(depth_to_obj_mean, 500-depth_to_obj_mean)
        depth_weight_norm /= tmp
        depth_weight_norm = torch.abs(depth_weight_norm)
        depth_weight_norm[torch.logical_not(mask_)] = 0.0

        depth_weight_norm = 1 + WEIGHTED*depth_weight_norm
        prediction = prediction 

        prediction = prediction.reshape(-1, weighted_num, h, w)[index_mask,...]
        depth_weight_norm = depth_weight_norm.reshape(-1, weighted_num, h, w)[index_mask,...]
        ...
        prediction = torch.sigmoid(prediction) * depth_weight_norm
