...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.depth_raw_extension = cfg.MODEL.DEPTH_MODALITY.RAW_IMG_EXTENSION
        ...
        if fusion_option=="mean":
            ...
            self.fusion_modal = ConcatModality() # TO-DO: find config param to pass to sub_feats for ConcatModality
            self.fusion_modal.to(self.device)
        elif fusion_option=="sae":
            self.fusion_modal = SAEFusion()
            self.fusion_modal.to(self.device)
        else:
            raise "There is no option {} for fusing color and depth".format(fusion_option)
