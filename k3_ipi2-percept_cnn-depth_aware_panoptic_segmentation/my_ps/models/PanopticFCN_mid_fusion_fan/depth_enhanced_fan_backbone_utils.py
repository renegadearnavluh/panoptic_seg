...
from detectron2.backbone import Backbone
...
from .fan_backbone_utils import FeatureAlign_V2, FeatureSelectionModule
...
def build_resnet_depth_enhanced_fan_backbone(cfg, should_forward_fused_features=False):
    input_shape_rgb = ShapeSpec(channels=len(cfg.MODEL.PIXEL_MEAN))
    input_shape_depth = ShapeSpec(channels=len(cfg.MODEL.DEPTH.PIXEL_MEAN))
    ...
    return DepthEnhancedFAN(bottom_up_rgb=bottom_up_rgb, 
